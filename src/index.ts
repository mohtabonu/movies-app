...
import {mainContainer, movBtnElm, movTitleElm, registerContainer, regUsername, regPassword, regName, loginContainer, loginUsername, loginPassword, movWrap, categoryWrap, searchElm } from './elements';
...
function renderMovies(selectedCategory = "all",searchQuery = '') {
  ...
  thead.innerHTML = `
      <tr class="bg-gray-100">
          <th class="py-3 px-5 border-b text-left">Title</th>
          <th class="py-3 px-5 border-b text-left">Genre</th>
          <th class="py-3 px-5 border-b text-left">Rate</th>
          <th class="py-3 px-5 border-b text-left"></th>
          <th class="py-3 px-5 border-b text-left"></th>
      </tr>
  `;
  ...
  for (const key in movies_information) {
      if (movies_information.hasOwnProperty(key)) {
          ...
          if ((selectedCategory === "all" || movie.category === selectedCategory) && movie.movieTitle.toLowerCase().includes(searchQuery.toLowerCase()) ) {
              ...
              tr.innerHTML = `
                  <td class="py-3 px-5 text-blue-600 cursor-pointer">${movie.movieTitle}</td>
                  <td class="py-3 px-5">${movie.category}</td>
                  <td class="py-3 px-5">${movie.rating}</td>
                  <td class="py-3 px-5">
                      <button>
                  ...
                  <td class="py-3 px-5">
                      <button class="bg-red-600 text-white px-3 py-1 rounded-md hover:bg-red-700">Delete</button>
                  </td>
              `;
...
searchElm.addEventListener("input", () => {
  renderMovies(undefined, searchElm.value);
});
